  ...
  private def showPersonDetails(person: Option[Person]): Unit =
    person match
      ...
      case None =>
        ...
        cityLabel.text = ""
        birthdayLabel.text = ""
  @FXML //everything in fxml file is java
  //def handleDeletePerson(action: ActionEvent): Unit = //import JavaFX for ActionEvent
    //val selectedIndex = personTable.selectionModel().selectedIndex.value
    //if (selectedIndex >= 0) then {
      //MainApp.personData.remove(selectedIndex)
      //personTable.items().remove(selectedIndex) this can replace Line 78
    //}
  /**
